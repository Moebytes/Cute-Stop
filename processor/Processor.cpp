...
auto Processor::prepareToPlay(double sampleRate, int samplesPerBlock) -> void {
    ...
    this->tapeStop.prepareToPlay(sampleRate);
}
...
auto Processor::processBlock(AudioBuffer<float>& buffer, [[maybe_unused]] MidiBuffer& midiMessages) -> void {
    ...
    this->tapeStop.setTrigger(this->parameters.trigger);
    this->tapeStop.setCurve(this->parameters.curve);

    for (int sample = 0; sample < buffer.getNumSamples(); sample++) {
        this->parameters.update();

        this->tapeStop.setStopTime(this->parameters.stopTime);
        this->tapeStop.setStartTime(this->parameters.startTime);

        this->tapeStop.processSample(inputL[sample], inputR[sample], outputL[sample], outputR[sample]);
    }
